...
namespace net
{
	...
	void wait(float seconds)			// this function suspends program for "seconds" amount of seconds
	{
...
#include <unistd.h>
	...
	// internet address
	// this seems to be a class that prepresents the address being accessed for the socket programming

	class Address
	{
	public:

		Address()			// this is a constructor for the Address class
		{
			address = 0;		// address and port are automatically 0 
			port = 0;
		}
		...
		unsigned int GetAddress() const		// get address
		{
			return address;
		}
		...
		bool operator == (const Address& other) const		// overloaded == operator
		{
	...
	public:
		...
		bool Open(unsigned short port)		// this opens the port 
		{
			assert(!IsOpen());
